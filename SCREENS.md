...
Alt tab bar: **Harcamalar | Liste | Kişiler**

Uygulama açılışta doğrudan **Harcamalar** sekmesine düşer (ayrı bir Ana Sayfa / özet ekranı yok).
...
Harcamalar (Tab, başlangıç ekranı)
  ├─ Harcama Listesi
  ├─ Harcama Detayı
...
Kişiler (Tab)
  └─ Kişiler (liste + kendi profilin + ev ayarları)
```
...
Profil ekranı ayrı değil — kullanıcının kendi bilgileri de bu ekranda, kendi satırı üzerinden yönetilir.

- Ev arkadaşlarının listesi: isim, IBAN (varsa).
- IBAN alanına dokununca panoya kopyalama.
- **Herkes sadece kendi satırını düzenleyebilir**: kendi adı, kendi IBAN'ı. Başkasının bilgisini göremeyecek şekilde kısıtlı değil — herkes herkesi görür, ama sadece kendi satırında "Düzenle" butonu aktiftir (RLS ile de garanti altına alınır, bkz. AGENTS.md).
- IBAN girilmemiş kişilerde "IBAN eklenmedi" etiketi.
- Ev ayarları bu ekranın alt kısmında / bir alt bölüm olarak yer alır:
